 ...
 * @LastEditTime: 2020-09-06 18:22:56
...
Rank Vector<T>::find(T const &e, Rank lo, Rank hi) const
{
	while ((lo < hi--) && (e != _elem[hi]))
		;	   //从后向前，顺序查找//利用短路特性，在hi=-1时也不会继续执行_elem[hi]
...
int Vector<T>::remove(Rank lo, Rank hi)
{
	...
	//@@@
	if (lo == hi)
		return 0; //处于效率考虑，单独考虑退化情况
	while (hi < _size)
		_elem[lo++] = _elem[hi++]; //[hi,_size)顺次前移.利用++更高效
...
{ //删除无序向量中重复的元素
	...
	while (i < _size)
	{
		(find(_elem[i], 0, i) < 0) ? i++ : remove(i);//选择性的i++，i不能一直++
